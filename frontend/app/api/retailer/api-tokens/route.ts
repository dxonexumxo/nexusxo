...
async function verifyRetailer(userId: string) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      if (!user) {
        ...
        if (authError || !userFromGetUser) {
          console.error('Auth error in GET /api/retailer/api-tokens:', authError || 'No session or user')
    ...
    const retailerCheck = await verifyRetailer(user.id)
    if (!retailerCheck.isValid) {
      return NextResponse.json({ error: 'Forbidden - Not a retailer' }, { status: 403 })
    }
    ...
    console.error('Unexpected error in GET /api/retailer/api-tokens:', error)
  ...
  try {
    ...
    if (!user) {
      ...
      if (!user) {
        const { data: { user: userFromGetUser }, error: authError } = await supabase.auth.getUser()
        if (authError || !userFromGetUser) {
          console.error('Auth error in POST /api/retailer/api-tokens:', authError || 'No session or user')
          return NextResponse.json({ error: 'Unauthorized', details: authError?.message || 'No session found' }, { status: 401 })
        }
        user = userFromGetUser
      }
    ...
    if (rateLimitError) {
      ...
      // Don't fail the request if rate limit creation fails - we can handle this later
    }
    ...
    console.error('Unexpected error in POST /api/retailer/api-tokens:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete rate limits first (cascade should handle this, but being explicit)
    ...
    console.error('Unexpected error in DELETE /api/retailer/api-tokens:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    console.error('Unexpected error in PATCH /api/retailer/api-tokens:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
