...
type Industry = 'Electronics' | 'Retail' | 'Food & Beverage' | 'Other'
...
export default function ManufacturerSignupPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Step 2: Insert manufacturer record into manufacturers table
      ...
      if (insertError) {
        // If insert fails, we should clean up the auth user, but for now just show error
        setError(`Failed to create manufacturer profile: ${insertError.message}`)
        setLoading(false)
        return
      }
